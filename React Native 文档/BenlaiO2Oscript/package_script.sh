...
#删除之前打包的文件
rm -r -f /Users/benlai/Desktop/Build/${appdirname}
...
if [ "$build_workspace" != "" ];then
	...
	build_cmd=${build_cmd}' -workspace '${build_workspace}' -scheme '${build_scheme}' -configuration '${build_config}' ONLY_ACTIVE_ARCH=NO'
...
#app文件名称
appname=$(basename /Users/benlai/Desktop/Build/${appdirname}/BenlaiO2O.app)
echo "appname" + ${appname}
#通过app文件名获得工程target名字
target_name=$(echo $appname | awk -F. '{print $1}')
echo "target_name" + ${target_name}
#app文件中Info.plist文件路径
app_infoplist_path=/Users/benlai/Desktop/Build/${appdirname}/${appname}/Info.plist
echo "app_infoplist_path" + ${app_infoplist_path}
#取版本号
...
#IPA名称
ipa_name="${target_name}_${bundleShortVersion}_${platform_id}_$(date +"%Y%m%d%H%M")"
...
#xcrun打包
xcrun -sdk iphoneos PackageApplication -v /Users/benlai/Desktop/Build/${appdirname}/*.app -o ${build_path}/ipa-build/${ipa_name}.ipa || exit
...
#zjw上传ipa文件, plist, ios.html
#sftp参数设置
sftp_server=192.168.60.225
sftp_username=root
...
publish_url="https://${publish_domain}/appdown/o2o-ios"

#html文件名
html_name="ios.html"

#ipa下载url
ipa_download_url=${publish_url}/${ipa_name}.ipa

#生成plist文件
cd $output_path
cat << EOF > ${ipa_name}.plist
       ...
       <dict>
           ...
           <array>
               ...
               <dict>
                   ...
                   <true/>
                   <key>url</key>
                   <string>${publish_url}/57.png</string>
               </dict>
	       <dict>
                   <key>kind</key>
                   ...
                   <true/>
                   <key>url</key>
                   <string>${publish_url}/512.png</string>
               </dict>
...
#生成html文件
# ${shell_path}/generate_html $output_path ${html_name} ${publish_url} ${ipa_name}

#上传ipa和plst文件
#${shell_path}/uploadItemsServicesFiles $output_path/${ipa_name} ${publish_domain} ${sftp_username} ${sftp_password} ${sftp_workpath} $output_path/${html_name} || exit

#触发服务器上的refresh_html脚本，更新ios.html
#${shell_path}/trigger_server_refresh_html ${sftp_workpath} ${publish_domain} ${sftp_username} ${sftp_password} 
